$(document).ready(function() {
    ...
    $.getJSON('pokemon.json', function(data) {
        ...
        console.log('Erreur lors du chargement du fichier JSON:', textStatus, error);
    });
    ...
    function displayResults(data) {
        resultsDiv.empty(); // Efface les résultats précédents
        const ul = $('<ul></ul>');

        data.forEach(function(pokemon) {
            const li = $('<li></li>').text(`${pokemon.name.english} (ID: ${pokemon.id}, Types: ${pokemon.type.join(', ')})`);
            ul.append(li);
        });
